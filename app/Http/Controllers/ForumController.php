...
use App\Http\Requests\Meili\ForumRequestBase;
...
{
    public function __invoke(ForumRequestBase $request,MeiliActionManager $manager): JsonResponse
    {
        //permission
        $ability = 'search-in-forum';

        //run action and (log binding in service provider)
        $itemsNULL =  $manager->driver('forum')->run(
        ...
        $success = [
            'result' => [
                'data'=>ForumResource::collection(collect($itemsNULL['data'])),
                'paginate'=>$itemsNULL['paginate'],
                'meta'=>(new ForumResource([]))->with($request),
            ]
        ];
